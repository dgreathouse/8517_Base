...
public class Drivetrain extends SubsystemBase implements IUpdateDashboard {
   ...
   * 
   * @param _speeds The ChassisSpeeds contains the X,Y direction and robot
   *                rotational speed
   */
  ...
  /**
   * Drive in a field centric mode. The X,Y are changed to compensate for the
   * Field orientation
   * 
   * @param _speeds The ChassisSpeeds contains the X,Y direction and robot
   *                rotational speed
  ...
  public void driveFieldCentric(ChassisSpeeds _speeds) {
    var robotCentricSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(_speeds, g.ROBOT.AngleActual_rot2d);
    var swerveStates = m_kinematics.toSwerveModuleStates(robotCentricSpeeds);
    setSwerveModules(swerveStates, true, true);
  }

  /**
   * Drive in a angle field centric mode. The X,Y are changed to compensate for
   * the Field orientation.
   * The rotational speed is changed to PID to the requested angle from
  ...
  /**
   * Drive in Polar Field Centric. This will drive at a specified angle, speed and
   * maintain the robot
   * heading based on g.ROBOT.AngleTarget_deg. This method is used in autonomous
   * when we only want
  ...
  /**
   * This is meant to cause the drive train to do a fast stop by changing the
   * wheels to ~45 degrees.
  ...
  public void fastStop() {
    switch (g.SWERVE.Count) {
      case 3:
        ...
        g.SWERVE.Modules[1].setDesiredState(new SwerveModuleState(0.0, new Rotation2d(Math.toRadians(-45))), true,
            true); // FR
      ...
      case 4:
        ...
        g.SWERVE.Modules[1].setDesiredState(new SwerveModuleState(0.0, new Rotation2d(Math.toRadians(-45))), true,
            true); // FR
        g.SWERVE.Modules[2].setDesiredState(new SwerveModuleState(0.0, new Rotation2d(Math.toRadians(135))), true,
            true); // BR
        g.SWERVE.Modules[3].setDesiredState(new SwerveModuleState(0.0, new Rotation2d(Math.toRadians(-135))), true,
            true); // BL
        break;
    }
  }
   ...
   * 
   * @param _states      An array of module speeds and angles developed from the
   *                     drive modes
   * @param _enableSteer enable the Steer Motor
   * @param _enableDrive enable the Drive Motor
   */
  ...
  /**
   * Set the drive speed multiplier for the amount of voltage to apply to the
   * drive motor.
  ...
  private class OdometryThread extends Thread {
    ...
    public void run() {
       ...
       * There may be a sync issue but when it comes to distances traveled it is less
       * than an inch.
       * Since the Yaw is Latenceny compensated the only issue left is Yaw vs Swerve
       * positions or Pose
       * Pose is updated at the same rate in this thread at the same rate as the
       * updated signals.
       * 
       */
      while (true) {
        ...
        m_yaw = m_pigeon2.getYaw();
        m_angularVelocityZ = m_pigeon2.getAccelerationZ();
  ...
  /**
   * Called by separate thread to put stuff to the dashboard at a slower rate than
   * the main periodic
   */
